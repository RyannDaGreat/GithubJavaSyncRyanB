...
public class HUHUHUrAutoUpdaterRefactorated
{
    ...
    private static String[] fullPathNamesOfDirectoryContents(java.io.File file)//Returns the names of all files in the directory
    {
    ...
    {
        //Does not work with java files that have packages
        File root=new File(srcFolderPath); // On Windows running on C:\, this is C:\java.
        LinkedList<String>javaFilePaths=new LinkedList<>();
        //noinspection ConstantConditions
        for(File x : root.listFiles())
            if((""+x).endsWith(".java"))
                javaFilePaths.add(""+x);
        ToolProvider.getSystemJavaCompiler().run(null,null,null,javaFilePaths.toArray(new String[javaFilePaths.size()]));//This is the part that might take a few seconds...
        return Class.forName(/*temporaryPackageName+"."+*/javaFileNameWithoutExtensionAndPackagePathSeparatedBySlashesInsteadOfPeriods_AllRelativeTo_srcFolderPath,true,URLClassLoader.newInstance(new URL[]{root.toURI().toURL()})); // Invokes the static initialization block. Should print "hello".;
    }
    ...
    public static void compileAndRunJavaCodeFromGithubRepository(String GitZipURL,String MainJavaClassFileNameWithoutExtension) throws ClassNotFoundException, IOException, InstantiationException, IllegalAccessException, InterruptedException
    {
        ...
        new File(getPath()+"/"+temporaryFolderName).mkdir();
        ...
        WriteFile(denchbunch+TemporaryClassName+".java",DesiredCode);//We're going to create a dummy class here. This class will simply call the main method of the class we want to start from via a static initialization block.
